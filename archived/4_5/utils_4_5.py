...
# ══════════════════════════════════════════════════════════════════════
# V7 Target Alignment (for fair cross-iteration comparison)
# ══════════════════════════════════════════════════════════════════════

def load_v7_targets(uid):
    """Read V7's per-user npz to extract exact (exp, date, pred_time) targets.

    Returns dict: {exp_name: [(date, pred_time, gt_geo_id), ...]}
    These can be passed to build_user_prompts(forced_targets=...) so V8
    predicts the EXACT same (user, date, time) as V7.
    """
    ...
    targets = {}  # {exp_name: [(date, pred_time, gt_geo_id), ...]}
    seen = set()
    for m in meta:
...
def build_user_prompts(user_df, uid, is_employed, forced_targets=None):
    ...
    Args:
        forced_targets: Optional dict {exp_name: [(date, pred_time, gt_geo), ...]}
            If provided, use these exact targets instead of random sampling.
            Used for V8 re-run aligned with V7 targets.
    ...
    for exp_name in exps:
        ...
        if forced_targets is not None and exp_name in forced_targets:
            # V8 aligned mode: use V7's exact targets
        ...
        for cfg_dict in CONFIG_MATRIX_CACHE[exp_name]:
            ...
            for tdate in target_dates:
                for pt in pred_times:
                    # Skip (date, pt) combos not in forced targets
