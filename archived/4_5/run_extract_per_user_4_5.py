...
def main():
    ...
    parser.add_argument('--align-v7', action='store_true',
                        help='Use V7 exact (date, pred_time) targets for fair comparison')
    args = parser.parse_args()
    ...
    # ── Load V7 targets if --align-v7 ──
    forced_targets = None
    if args.align_v7:
        forced_targets = load_v7_targets(uid)
        if forced_targets is None:
            print(f"ERROR: V7 per_user file not found for {uid}")
            print(f"  Expected: outputs_v7/per_user/{uid}.npz")
            return
        for exp, tgts in forced_targets.items():
            dates = sorted(set(d for d, _, _ in tgts))
            print(f"  [align-v7] {exp}: {len(dates)} dates, "
                  f"{len(tgts)} (date,time) pairs")
